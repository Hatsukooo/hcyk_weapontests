...
const WeaponTest = () => {
  ...
  const [results, setResults] = useState<TestResults>({ passed: false, score: 0, wrongAnswers: [] });
  ...
  const shuffleOptionsInQuestions = (questions: Question[]): Question[] => {
    return questions.map(question => {
      // Create pairs of options and their corresponding letter values
      const optionPairs: QuestionOption[] = question.Question_options.map((option: string, index: number) => {
        return {
          text: option,
  ...
  useEffect(() => {
    const getQuestionsData = async () => {
      try {
        ...
        const response = await fetchNui<{questions: Question[]}>("getQuestions", {}, {
          questions: [
            {
              Question_Label: "When you hold a weapon you cannot...",
              Question_options: ["Run", "Point it at civilians", "Reload", "Change weapons"],
  ...
  const getOptionText = (question: Question | undefined, optionKey: string): string => {
